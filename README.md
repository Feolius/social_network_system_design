...
Картинки в посте занимают 4 * 10 = 40 mb.

_traffic (read картинки) = 2000 * 40 mb = 80 gb/s_

Трафик на запись оценю через соотношение RPS

_traffic (write) = 24 * (30 / 2000) = 0.4 mb/s_
_traffic (write картинки) = 80 * (30 / 2000) = 1 gb/s_
...
Кроме тех операций, что описаны выше, пользователь также может создавать профили, смотреть профили других людей, подписываться и отписываться от людей. Но в соответствии с нефункциональными требованиями, эта активность происходит на порядки реже, поэтому в данной оценке мы можем пренебречь. 